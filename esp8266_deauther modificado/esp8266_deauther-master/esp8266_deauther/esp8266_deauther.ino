...
//Anadido------------------------------------------------------------------------------------------------------------
//CapacitiveSensor   cs_D8_D3 = CapacitiveSensor(D8,D3);        // 10M resistor between pins 4 & 2, pin 2 is sensor pin, add a wire and or foil if desired
//int estado = 0;
...
void startAPScan() {
  scanMode = "scanning...";
#ifdef USE_DISPLAY // ifdef - definicioón de variable
  drawInterface(); // clic en la web, varia un valor y rediseña la web entera (actualiza)
#endif
  if (apScan.start()) {
...
void sendAPResults() { //envía los resultados del scan
  apScan.sendResults(); 
}
...
//==========Attack================================================================================================================================
void sendAttackInfo() {
  attack.sendResults(); // imprime resultado de lo que hace en el monitor
}
...
//==========Settings=====================================================================================================Carga toda la config del sistema
void getSettings() {
  settings.send();
}
...
void setup() { //-----------------------------------------------------------------------------------------------------------------------
  ...
  //Anadido---------------------------------------------------------------------------------------------------------------anadido
  //pinMode(D8, OUTPUT);
   //cs_D8_D3.set_CS_AutocaL_Millis(0xFFFFFFFF);     // turn off autocalibrate on channel 1 - just as an example
  ...
  startWifi(); // activa la wifi
  attack.stopAll(); //detiene los ataques que puedan haber 
  attack.generate(); //obtiene la MAC de los cliente conectados a la wifi
...
void loop() {
  ...
  //Prueba ----------------------------------------------------------------------------parte anadida , capacitador apaga y enciende al tocar--------------------------------------------------------------------------------
  //long start = millis();
  ...
  //if (total1 > 10 && estado == 0){
    ...
    settings.multiAPs = true; //se habilita poder hacer una selección múltiple de los puntos de acceso
    for (int i = 0; i < apScan.results; i ++) { // a partir de la lista, reaaliza una acción con cada punto de acceso independientemente
      apScan.select(i); // esta es la acción del for, es la selección de los puntos de acceso de modo automático 
    }
    attack.stopAll(); 
    attack.start(0);
    startAttack();
 ...
 /* if(estado == 1){
  ...
  if (clientScan.sniffing) { // sniffing - rastrea las conexiones de clientes
    if (clientScan.stop()) startWifi(); //si para el escan se activa el wifi
  } else {
